function build_circos_input(Domain_list, Peptide_list, replicate_Merged_vectors, interaction_Kd)
%Greg Koytiger June 2012
%Converts Adaptor-Adaptor interaction data into a format that can be
%rendered by Circos

%%Eliminates tandem domains
...
%isBound = false(size(Peptide_list,1),1); %%
...
for i = 1:size(replicate_Merged_vectors,1)
    ...
    for j = 1:length(pep_idx)
        if(ismember(Peptide_list(pep_idx(j),1), Domain_list(:,2)))
        %if(strcmp(Peptide_list(pep_idx(j),1), 'IRS1') || (ismember(Peptide_list(pep_idx(j),1), Domain_list(:,2)) && strcmp(Domain_list(domain_idx,2), 'IRS1')))%%
            %isBound(pep_idx) = true;
...
%Linkstarts = Peptide_list(ismember(Peptide_list(:,1), Domain_list(:,2)), 1:3);
% Linkstarts = Peptide_list(ismember(Peptide_list(:,1), Domain_list(:,2)) & isBound, 1:3);
% 
% %%Createst index of link starts so they can be marked
% for i = 1:size(Linkstarts,1)
%     temp = str2double(Linkstarts{i,2}(2:end));
%     Linkstarts{i,2} = temp - 5; Linkstarts{i,3} = temp + 5;
% end
...
Linkends_sh2 = Domain_list(strcmp(Domain_list(:,6), 'SH2'), [2,9:10]);
Linkends_ptb = Domain_list(strcmp(Domain_list(:,6), 'PTB'), [2,9:10]);

xlswrite('output/circos/links.xls', Links_list);  %xlswrite('output/circos/linkstarts.xls', Linkstarts);
xlswrite('output/circos/Linkends_sh2.xls', Linkends_sh2);
xlswrite('output/circos/Linkends_ptb.xls', Linkends_ptb);
